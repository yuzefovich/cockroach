...
func TestStepper(t *testing.T) {
	...
	for _, c := range testCases {
		t.Run("", func(t *testing.T) {
			...
			d, err := NewStepper(&flowCtx, 0 /* processorID */, &ss, in, &distsqlpb.PostProcessSpec{}, out)
			if err != nil {
				t.Fatal(err)
			}
...
func benchmarkStepper(b *testing.B, step uint32) {
	const numCols = 2
	...
	for _, numRows := range []int{1 << 4, 1 << 8, 1 << 12, 1 << 16} {
		b.Run(fmt.Sprintf("rows=%d", numRows), func(b *testing.B) {
			...
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				d, err := NewStepper(flowCtx, 0 /* processorID */, spec, input, post, &RowDisposer{})
				if err != nil {
					b.Fatal(err)
				}
				d.Run(context.Background())
